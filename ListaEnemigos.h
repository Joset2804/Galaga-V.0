...
public:
	...
	void MostrarEnemigos(Graphics^ g, int nivel);
	void DisparoEnemigos(Graphics^ g, int nivel);
private:
	NodoEnemigo* primero;
	NodoEnemigo* último;
	int N;
...
{
	auto añadir_enemigo = [&](Graphics^ g, int cant, int nivel) //SEGUNDO LAMBDA
	{
...
inline void ListaEnemigos::MostrarEnemigos(Graphics^ g, int nivel)
{
	EliminarEnemigos();
	NodoEnemigo* actual = primero;
	if (primero != NULL)
	{
		while (actual != NULL)
		{
			actual->objEnemigo->Mostrar(g);
			//actual->objEnemigo->GetListaBalas()->MostrarBalas(g, 2);
			actual = actual->siguiente;
		}
	}
}

inline void ListaEnemigos::DisparoEnemigos(Graphics^ g, int nivel)
{
	EliminarEnemigos();
	NodoEnemigo* actual = primero;
	if (primero != NULL)
